...
fn criterion_benchmark(c: &mut Criterion) {
	...
	c.bench_function("image_brightness sample 1", |b| {
		b.iter(|| image_brightness::image_brightness(&image_data, 1).unwrap())
	});
	c.bench_function("image_brightness sample 10", |b| {
		b.iter(|| image_brightness::image_brightness(&image_data, 10).unwrap())
	});
	c.bench_function("image_brightness sample 100", |b| {
		b.iter(|| image_brightness::image_brightness(&image_data, 100).unwrap())
	});
}
